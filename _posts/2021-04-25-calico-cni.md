...
[转载自jimmysong](https://jimmysong.io/kubernetes-handbook/concepts/cni.html)
...
Kubernetes Pod 中的其他容器都是Pod所属pause容器的网络，创建过程如下：

1. kubelet县创建pause容器生成network  namespace
2. 调用网络CNI driver
3. CNI driver 根据配置调用具体的cni插件
4. cni插件给pause容器配置网络
5. pod中其他的容器都适用pause容器的网络
